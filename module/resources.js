class Resources extends Application {
    ...
    async toggleResources(){
      ...
      if (this.isOpen) {
        ...
        this.content = await renderTemplate(templatePath, starshipResources);
      ...
      html.find(displays).click(ev => {
        ...
        if (game.user.isGM) {
            ...
            if (resName !== undefined && resName !== null) {
                this.createDialog(resName, starshipResources[resName]);
            }
        }
      });

      html.find(resourcesMove).mousedown(ev => {
        ev.preventDefault();
        ev = ev || window.event;
        let isRightMB = false;
